function ElectricalAppliance(name, hierarchy) {
    this.name = name,
    this.hierarchy = hierarchy
}

ElectricalAppliance.prototype.calculatePower = function (kWt, h) {
    let e = kWt * h;
    console.log(`Потребляемая мощность прибора ${e} кВт`);
}

ElectricalAppliance.prototype.inSocket = function (bool) {
    if (bool)
        console.log('Прибор включен в розетку');
    else console.log('Прибор не подключен к розетке');
}

function ApplianceType(name, type) {
    this.name = name,
    this.type = type
}

ApplianceType.prototype = new ElectricalAppliance();

ApplianceType.prototype.inSocket = function (bool) {
    if (bool)
